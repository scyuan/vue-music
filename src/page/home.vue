      ...
      <div class="discover-wrap">
        <div class="recommend-gedan">
          ...
          <ul class="gedan-content clearfix" ref="gedan_content">
            <li v-for="(item,index) in gedanList" :key="index">
              <img
                v-lazy="item.picUrl"
                v-on:load="imgload(index,item.picUrl)"
                v-on:click="gotogedan(item.id)"
                alt
              />
              <p class="gedan-name">{{item.name}}</p>
              <span class="playCount">
                <i class="icon">&#xe63f;</i>
                {{parseToWan(item.playCount)}}
              </span>
            </li>
          </ul>
        </div>
...
export default {
  ...
  mounted: function() {
    ...
    //获取轮播图片---暂时还不能自动播放
    this.$http.get("http://120.79.167.62:3000/banner").then(
      response => {
        var banners = response.data.banners;
        // console.log(111, banners);
        this.imgUrls = banners.map(banner => {
    ...
    //获取歌单数据
    this.$http.get("http://120.79.167.62:3000/personalized").then(
      res => {
    ...
    //获取歌曲数据
    this.$http.get("http://120.79.167.62:3000/personalized/newsong").then(
      res => {
